{ config, lib, ... }: let
	...
	# Hyprland check for keyboard layout configuration.
...
in rec {
	...
	home-manager.users.${config.userName}.wayland.windowManager.hyprland.settings.input = lib.optionalAttrs hyprland {
		# Keyboard layout to use.
