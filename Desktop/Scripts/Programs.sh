...
[ "$Program" = " " ] &&
	bash "/etc/nixos/Desktop/Scripts/Program launcher.sh"
...
[ "$Program" = "󰋊  Gparted              Partition manager" ] &&
	$TERMINAL -e sudo -E gparted & disown

[ "$Program" = "󰋊  ncdu                 Disk usage" ] &&
	$TERMINAL -e ncdu & disown

[ "$Program" = "  Ventoy               Bootable USB creation tool" ] &&
	$TERMINAL -e sudo ventoy-web & disown
...
[ "$Program" = "  Hyprpaper            Set desktop background/wallpaper" ] &&
	bash "/etc/nixos/Desktop/Scripts/Hyprpaper.sh" & disown
