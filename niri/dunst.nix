...
{ home-manager.users.${config.userName}.services.dunst = {
	...
	# Use the icon theme defined for the user in the `/etc/nixos/theming/icons.nix` module.
	iconTheme = {
		package = config.home-manager.users.${config.userName}.gtk.iconTheme.package;
		name = config.home-manager.users.${config.userName}.gtk.iconTheme.name;
	};
	...
	settings = {
		...
		global = rec {
			...
			# Path of the icon set to be used, defined in the `/etc/nixos/theming/icons.nix` module.
#			icon_path = "$HOME/.nix-profile/share/icons/${config.home-manager.users.${config.userName}.gtk.iconTheme.name}/status/scalable/16/:$HOME/.nix-profile/share/icons/${config.home-manager.users.${config.userName}.gtk.iconTheme.name}/devices/scalable/";
			...
			# The `$BROWSER` variable is defined in the `/etc/nixos/programs/internet.nix` module.
			browser = "$BROWSER";
