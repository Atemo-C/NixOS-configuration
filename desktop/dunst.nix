{ config, lib, ... }: lib.mkIf config.programs.niri.enable {
	home-manager.users.${config.userName} = rec {
		...
		services.dunst.iconTheme.package = lib.mkIf config.home-manager.users.${config.userName}.gtk.iconTheme.package;
		services.dunst.iconTheme.name = lib.mkIf config.home-manager.users.${config.userName}.gtk.iconTheme.name;
		...
		systemd.user.tmpfiles.rules = lib.optional services.dunst.enable
